...
public class AList<T extends Comparable<? super T>> implements ListInterface<T> {
    ...
    public int removeEvery(T element) {
        int removed = 0;
        int tempListNumberOfEntries = 0;
        T[] tempList = (T[]) new Comparable[numberOfEntries + 1];

        for (int i = 1; i <= numberOfEntries; i++) {
            if (list[i] != null && !list[i].equals(element)) {
                tempList[tempListNumberOfEntries + 1] = list[i];
                tempListNumberOfEntries++;
            }
        ...
        numberOfEntries = tempListNumberOfEntries;
        list = tempList;
        return removed;
    }
